...
class OAuth2Credentials(Credentials):
  ...
  def authorize(self, http):
    ...
    def new_request(uri, method='GET', body=None, headers=None,
      ...
      if not self.access_token:
        logger.info('OAuth2Credentials.authorize.new_request: Attempting refresh to obtain initial access_token')
      ...
      if resp.status in [401, 403]:
        logger.info('OAuth2Credentials.authorize.new_request: Refreshing due to a %s' % str(resp.status))
        self._refresh(request_orig)
        self.apply(headers)
  ...
  def access_token_expired(self):
    ...
    if now >= self.token_expiry:
      logger.info('OAuth2Credentials.access_token_expired: access_token is expired. Now: %s, token_expiry: %s',
    ...
    if not self.store:
      ...
      try:
        ...
        if (new_cred and not new_cred.invalid and
          ...
          logger.info('OAuth2Credentials._refresh: Updated access_token read from Storage')
  ...
  def _do_refresh_request(self, http_request):
    ...
    logger.info('OAuth2Credentials._do_refresh_request: Refreshing access_token')
    ...
    if resp.status == 200:
      ...
      logger.info('OAuth2Credentials._do_refresh_request: Failed to retrieve access token: %s' % content)
...
if HAS_OPENSSL:
  ...
  class SignedJwtAssertionCredentials(AssertionCredentials):
    ...
    def _generate_assertion(self):
      ...
      logger.debug("SignedJwtAssertionCredentials._generate_assertion ==>")
...
class OAuth2WebServerFlow(Flow):
  ...
  def step2_exchange(self, code, http=None):
    ...
    if resp.status == 200 and 'access_token' in d:
      ...
      logger.info('OAuth2WebServerFlow.step2_exchange: Successfully retrieved access token')
      return OAuth2Credentials(access_token, self.client_id,
                               self.client_secret, refresh_token, token_expiry,
                               self.token_uri, self.user_agent,
                               id_token=d.get('id_token', None))
    else:
      logger.info('OAuth2WebServerFlow.step2_exchange: Failed to retrieve access token: %s' % content)
      if 'error' in d:
